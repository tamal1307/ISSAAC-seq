...
# Read RNA data
mat_rep1_RNA <- Read10X(data.dir = "/path/to/mCortex_rep1_RNA/Solo.out/Gene/filtered/")
mat_rep2_RNA <- Read10X(data.dir = "/path/to/mCortex_rep2_RNA/Solo.out/Gene/filtered/")
mat_all_RNA <- cbind(mat_rep1_RNA, mat_rep2_RNA[rownames(mat_rep1_RNA),])

# Read ATAC data
mat_all_ATAC <- Read10X_h5("/path/to/mCortex_ATAC_aggr/outs/filtered_peak_bc_matrix.h5")
...
frag.file <- "/path/to/mCortex_ATAC_aggr/outs/fragments.tsv.gz"
...
saveRDS(mCortex_all, "mCortex_all_original.rds")

# Export expression matrix
expression_matrix <- mCortex_all@assays$RNA@counts
write.csv(expression_matrix, "mCortex_all_expression_matrix.csv", row.names = T, col.names = T, quote=F)

#===============RNA analysis

mCortex_all <- readRDS("mCortex_all_original.rds")
...
write.csv(mCortex_all@reductions$umap.rna@cell.embeddings, "mCortex_all_RNA_UMAP_coordinates.csv", row.names = T, col.names = T, quote=F)

# Find different genes
Idents(mCortex_all) <- "RNA_snn_res.1"
diff.wilcox = FindAllMarkers(mCortex_all)
write.csv(diff.wilcox, "mCortex_all_diff_genes_wilcox_r1.0.csv", row.names = F)
...
write.csv(mCortex_all@reductions$umap.atac@cell.embeddings, "mCortex_all_ATAC_UMAP_coordinates.csv", row.names = T, col.names = T, quote=F)

#Find different peaks
diff.LR_peaks = FindAllMarkers(mCortex_all, assay = 'ATAC',test.use = "LR")
write.csv(diff.LR_peaks, "mCortex_all_diff_peaks_LR_r1.0.csv", row.names = F)
...
allen <- readRDS("allen_mCortex.rds")
...
saveRDS(mCortex_all, "mCortex_all.final.rds")

# Export metadata
write.csv(mCortex_all@meta.data, "mCortex_all_metadata.csv", quote = FALSE)
...
mCortex_all <- readRDS("mCortex_all.final.rds")
DefaultAssay(mCortex_all) <- "RNA"
...
write.csv(diff.wilcox_mCortex_all_R14_R20, "diff.wilcox_mCortex_all_R14_R20.csv")
...
write.csv(peudotime_inf, "mCortex_all_OPC_to_Oligo_peudotime_information.csv")
...
write.csv(genSmoothCurves_mat_RNA, "mCortex_all_OPC_to_Oligo_cor_gene_expression_pseudotime.csv")
...
write.csv(genSmoothCurves_mat_activity, "mCortex_all_OPC_to_Oligo_cor_gene_activity_score_pseudotime.csv")
